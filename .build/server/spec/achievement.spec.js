...
var import_achievement = __toESM(require("../services/achievement"));
const userId = "abc";
  ...
  const incrementAchievementTest = async (key, incrementAmount, incrementFunctionCallback) => {
    ...
    const fakeUserRepo = {
      async updateOne(query, update) {
        if (query._id === userId && update.$inc[key] == incrementAmount) {
          calls++;
        }
      }
    };
    const fakeGuildService = {};
    let service = new import_achievement.default(fakeUserRepo, fakeGuildService);
...
//# sourceMappingURL=achievement.spec.js.map
