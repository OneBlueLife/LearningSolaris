...
var gameHelper_exports = {};
__export(gameHelper_exports, {
  default: () => gameHelper_default
});
module.exports = __toCommonJS(gameHelper_exports);
var import_moment = __toESM(require("moment"));
var import_diplomacyHelper = __toESM(require("./diplomacyHelper"));
class GameHelper {
  getUserPlayer(game) {
  ...
  getCarriersOrbitingStar(game, star) {
    return game.galaxy.carriers.filter((x) => x.orbiting === star._id).sort((a, b) => (a.ticksEta || 0) - (b.ticksEta || 0));
  }
  ...
  getTicksBetweenLocations(game, carrier, locs, tickDistanceModifier = 1) {
    ...
    for (let i = 1; i < locs.length; i++) {
      ...
      let ticks;
      if (prevLoc.type === "star" && currLoc.type === "star" && prevLoc.object.wormHoleToStarId === currLoc.object._id && currLoc.object.wormHoleToStarId === prevLoc.object._id) {
        ticks = 1;
      } else {
  ...
  getCountdownTime(game, date) {
    ...
    let relativeTo = (0, import_moment.default)().utc();
    let t = (0, import_moment.default)(date).utc() - relativeTo;
    return t;
  }
  ...
  getCountdownTimeStringByTicks(game, ticks, useNowDate = false, largestUnitOnly = false) {
    ...
    if (game.settings.gameTime.gameType === "realTime") {
      let date = useNowDate ? (0, import_moment.default)().utc() : game.state.lastTickDate;
  ...
  getDateToString(date, largestUnitOnly = false) {
    let days = Math.floor(date / (1e3 * 60 * 60 * 24));
    let hours = Math.floor(date % (1e3 * 60 * 60 * 24) / (1e3 * 60 * 60));
    let mins = Math.floor(date % (1e3 * 60 * 60) / (1e3 * 60));
    let secs = Math.floor(date % (1e3 * 60) / 1e3);
    if (secs < 0) {
      return "Pending...";
    }
    let str = "";
  ...
  getCountdownTimeForTurnTimeout(game) {
    return (0, import_moment.default)(game.state.lastTickDate).utc().add("minutes", game.settings.gameTime.maxTurnWait);
  }
  ...
  calculateTimeByTicks(ticks, speedInSeconds, relativeTo = null) {
    if (relativeTo == null) {
      relativeTo = (0, import_moment.default)().utc();
    } else {
      relativeTo = (0, import_moment.default)(relativeTo).utc();
    }
  ...
  canTravelAtWarpSpeed(game, player, carrier, sourceStar, destinationStar) {
    ...
    if (sourceStar.warpGate && destinationStar.warpGate && sourceStar.ownedByPlayerId && destinationStar.ownedByPlayerId) {
      let sourceAllied = sourceStar.ownedByPlayerId === carrier.ownedByPlayerId || import_diplomacyHelper.default.isFormalAlliancesEnabled(game) && import_diplomacyHelper.default.isDiplomaticStatusToPlayersAllied(game, sourceStar.ownedByPlayerId, [carrier.ownedByPlayerId]);
      let desinationAllied = destinationStar.ownedByPlayerId === carrier.ownedByPlayerId || import_diplomacyHelper.default.isFormalAlliancesEnabled(game) && import_diplomacyHelper.default.isDiplomaticStatusToPlayersAllied(game, destinationStar.ownedByPlayerId, [carrier.ownedByPlayerId]);
  ...
  isStarPairWormHole(sourceStar, destinationStar) {
    return sourceStar && destinationStar && sourceStar.wormHoleToStarId && destinationStar.wormHoleToStarId && sourceStar.wormHoleToStarId === destinationStar._id && destinationStar.wormHoleToStarId === sourceStar._id;
  }
  ...
  isGameInProgress(game) {
    return !this.isGameWaitingForPlayers(game) && !this.isGamePaused(game) && game.state.startDate != null && (0, import_moment.default)().utc().diff(game.state.startDate) >= 0 && !game.state.endDate;
  }
  isGamePendingStart(game) {
    return !this.isGameWaitingForPlayers(game) && !this.isGamePaused(game) && game.state.startDate != null && (0, import_moment.default)().utc().diff(game.state.startDate) < 0;
  }
  isGameFinished(game) {
  ...
  isDarkMode(game) {
    return game.settings.specialGalaxy.darkGalaxy === "standard" || game.settings.specialGalaxy.darkGalaxy === "extra" || game.settings.specialGalaxy.darkGalaxy === "start" && game.state.startDate == null;
  }
  isDarkFogged(game) {
  ...
  playerHasLowestTechLevel(game, techKey, player) {
    const levels = [...new Set(game.galaxy.players.filter((p) => p.research != null).map((p) => {
  ...
  getSortedLeaderboardPlayerList(game) {
    ...
    const sortPlayers = (a, b) => {
      ...
      if (a.defeated && b.defeated) {
        if ((0, import_moment.default)(a.defeatedDate) > (0, import_moment.default)(b.defeatedDate))
          return -1;
        if ((0, import_moment.default)(a.defeatedDate) < (0, import_moment.default)(b.defeatedDate))
          return 1;
      }
    ...
    if (this.isGameFinished(game)) {
      let undefeatedLeaderboard = playerStats.filter((x) => !x.defeated).sort(sortPlayers);
      let defeatedLeaderboard = playerStats.filter((x) => x.defeated).sort(sortPlayers);
  ...
  getDateString(date) {
    date = (0, import_moment.default)(date).utc().toDate();
  ...
  getGamePlayerShapesCount(game) {
    return (/* @__PURE__ */ new Set([...game.galaxy.players.map((p) => p.shape)])).size;
  }
  isRealTimeGame(game) {
  ...
  canTick(game) {
    ...
    let lastTick = (0, import_moment.default)(game.state.lastTickDate).utc();
    let nextTick;
    if (this.isRealTimeGame(game)) {
      nextTick = (0, import_moment.default)(lastTick).utc().add(game.settings.gameTime.speed, "seconds");
    } else if (this.isTurnBasedGame(game)) {
      let isAllPlayersReady = this.isAllUndefeatedPlayersReady(game);
      if (isAllPlayersReady) {
        return true;
      }
      nextTick = (0, import_moment.default)(lastTick).utc().add(game.settings.gameTime.maxTurnWait, "minutes");
    } else {
      throw new Error(`Unsupported game type.`);
    }
    return nextTick.diff((0, import_moment.default)().utc(), "seconds") <= 0;
  }
  ...
  isInGuild(guild, userId) {
    return guild.members.find((m) => m._id === userId) || guild.officers.find((m) => m._id === userId) || guild.leader && guild.leader._id === userId;
  }
  isUserSpectatingGame(game) {
    return !this.getUserPlayer(game);
  }
...
var gameHelper_default = new GameHelper();
