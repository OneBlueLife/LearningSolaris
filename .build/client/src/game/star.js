...
class Star extends import_events.default {
    ...
    if (this.data.isInScanningRange) {
      if (this.isBinaryStar()) {
        if (this.hasBlackHole()) {
          this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["black_hole_binary"]);
        } else if (this._isDeadStar()) {
          this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["binary_unscannable"]);
        } else {
          this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["binary_scannable"]);
        }
      } else if (this.hasBlackHole()) {
        this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["black_hole"]);
      } else if (this.data.homeStar) {
        this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["home"]);
      } else if (this._isDeadStar()) {
        this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["unscannable"]);
      } else {
        this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["scannable"]);
      }
    } else {
      this.graphics_star = new PIXI.Sprite(import_texture.default.STAR_SYMBOLS["unscannable"]);
      this.graphics_star.tint = 10526880;
    }
    this.graphics_star.anchor.set(0.5);
    this.graphics_star.width = 24 / 2;
    this.graphics_star.height = 24 / 2;
    this.container.addChild(this.graphics_star);
  }
  drawPulsar() {
    if (!this.isPulsar()) {
    ...
    let playerColour = player ? player.colour.value : 16777215;
    ...
    this.pulsarGraphics.drawEllipse(8, 0, 8, 8);
    this.pulsarGraphics.rotation = Star.seededRNG.random() * Math.PI * 2;
  ...
  drawNebula() {
    ...
    let seed = this.data._id;
    Star.seededRNG.seed(seed);
    let nebulaTexture = import_texture.default.getRandomStarNebulaTexture(seed);
    ...
    this.nebulaSprite.rotation = Star.seededRNG.random() * Math.PI * 2;
    let player = this._getStarPlayer();
    let playerColour = player ? player.colour.value : 16777215;
    this.nebulaSprite.tint = playerColour;
    let blendSprite = new PIXI.Sprite(nebulaTexture);
    blendSprite.anchor.set(0.5);
    blendSprite.rotation = Star.seededRNG.random() * Math.PI * 2;
  ...
  drawWormHole() {
    ...
    let texture = import_texture.default.getRandomWormholeTexture();
    ...
    this.wormHoleSprite.rotation = Math.random() * Math.PI * 2;
  ...
  drawAsteroidField() {
    ...
    let seed = this.data._id;
    Star.seededRNG.seed(seed);
    let texture = import_texture.default.getRandomStarAsteroidFieldTexture(seed);
    ...
    this.asteroidFieldSprite.rotation = Star.seededRNG.random() * Math.PI * 2;
    let player = this._getStarPlayer();
    let playerColour = player ? player.colour.value : 16777215;
  ...
  drawPlanets() {
    ...
    if (!this.container_planets) {
      ...
      for (let i = 0; i < planetCount; i++) {
        ...
        orbitGraphics.lineStyle(0.3, 16777215);
  ...
  drawNaturalResourcesRing() {
    ...
    for (let lod = 0; lod < Star.maxLod; lod += 1) {
      ...
      lineWidht *= lod + 1;
      this.graphics_natural_resources_ring[lod].clear();
      this.graphics_natural_resources_ring[lod].lineStyle(lineWidht, 16777215, 0.1);
      this.graphics_natural_resources_ring[lod].drawCircle(0, 0, ringRadius * 0.75);
      this.graphics_natural_resources_ring[lod].scale.x = 1 / (1 / 8 * (lod + 1));
      this.graphics_natural_resources_ring[lod].scale.y = 1 / (1 / 8 * (lod + 1));
  ...
  _getPlanetsCount() {
    ...
    let averageNaturalResources = this._calculateAverageNaturalResources(this.data.naturalResources);
    return Math.min(Math.floor(averageNaturalResources / 45 * 3), 5);
  }
  _getPlanetOrbitDirection() {
    return Math.floor(Math.abs(this.data.location.y)) % 2 === 0;
  }
  _getPlanetOrbitSpeed() {
    return Math.floor(Math.random() * (1e3 - 500 + 1) + 500);
  }
  drawColour() {
    ...
    if (Object.keys(import_texture.default.PLAYER_SYMBOLS).includes(player.shape)) {
      this.graphics_shape_part = new PIXI.Sprite(import_texture.default.PLAYER_SYMBOLS[player.shape][2 + this.data.warpGate]);
      this.graphics_shape_full = new PIXI.Sprite(import_texture.default.PLAYER_SYMBOLS[player.shape][0 + this.data.warpGate]);
    }
    ...
    this.graphics_shape_part.width = 28;
    this.graphics_shape_part.height = 28;
    this.graphics_shape_full.width = 28;
    this.graphics_shape_full.height = 28;
    this.container.addChild(this.graphics_shape_part);
    this.container.addChild(this.graphics_shape_full);
  }
  _hasUnknownShips() {
  ...
  drawName() {
    ...
    if ((this.data.ownedByPlayerId || carriersOrbiting) && (totalKnownShips > 0 || carriersOrbiting.length > 0 || this._hasUnknownShips())) {
      this.text_name.y = (Star.nameSize + Star.shipsSmallSize) / 2 - Star.nameSize;
    } else {
  ...
  drawShips() {
    ...
    if (this.data.ownedByPlayerId || carriersOrbiting) {
      ...
      if (carrierCount) {
        ...
        if (import_gameHelper.default.isStarHasMultiplePlayersInOrbit(this.game, this.data)) {
          shipsText += "+";
        }
      }
    }
    if (shipsText) {
      if (!this.text_ships_small) {
        ...
        this.text_ships_small.y = -this.text_ships_small.height + ((Star.nameSize + Star.shipsSmallSize) / 2 - Star.nameSize);
      }
      if (!this.text_ships_big) {
        ...
        this.text_ships_big.y = -this.text_ships_big.height / 2;
      }
  ...
  drawInfrastructureBulkIgnored() {
    ...
    if (!this.text_infrastructureBulkIgnored) {
      ...
      this.text_infrastructureBulkIgnored.x = -(this.text_infrastructureBulkIgnored.width / 2);
  ...
  drawScanningRange() {
    ...
    this.graphics_scanningRange.lineStyle(1, 16777215, 0.2);
  ...
  drawHyperspaceRange() {
    ...
    this.graphics_hyperspaceRange.lineStyle(1, 16777215, 0.2);
  ...
  drawTarget() {
    ...
    if (this.data.targeted) {
      this.graphics_targeted.lineStyle(2, 16711680);
  ...
  drawKingOfTheHillCircle() {
    ...
    if (this.data.isKingOfTheHillStar) {
      this.graphics_kingOfTheHill.lineStyle(0.5, 16777215);
      this.graphics_kingOfTheHill.alpha = 0.5;
      this.graphics_kingOfTheHill.drawCircle(0, 0, 20);
    }
  }
  drawDepth() {
    const depth = import_helpers.default.calculateDepthModifier(this.userSettings, this.data._id);
  ...
  updateVisibility() {
    let aparentScale = this.container.scale.x * (this.zoomPercent / 100);
    let lod = Math.max(Math.min(Math.floor(aparentScale) - 1, Star.maxLod - 1), 0);
    ...
    if (this.container_planets)
      this.container_planets.visible = this.data.isInScanningRange && this.zoomPercent >= Star.zoomLevelDefinitions.naturalResources;
    if (this.text_infrastructure)
      this.text_infrastructure.visible = this.isSelected || this.zoomPercent >= Star.zoomLevelDefinitions.infrastructure;
    ...
    let partial_ring = this.text_ships_big && this.text_ships_big.visible || this.text_ships_small && this.text_ships_small.visible || this.text_name && this.text_name.visible;
